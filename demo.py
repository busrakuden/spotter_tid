...
def main(
    ...
    fps: int = 25,
    embd_dim: int = 256,
    dict_video_path: str = "../bsldict/videos/",
):
    """
    ...
    dict_ix = np.where(np.array(bsldict_metadata["glosses"]) == keyword)[0]
    print(f"Found {len(dict_ix)} dictionary videos for the keyword {keyword}.")
    dict_features = np.array(bsldict_metadata["embeddings"])[dict_ix]
    ...
    for b in range(num_batches):
        ...
        # Forward pass
        out = model(inp)
        print(out["embds"].shape)
        continuous_features.append(out["embds"].detach().cpu().numpy())
        
    continuous_features = np.vstack(continuous_features)
    assert continuous_features.shape[0] == num_clips
    ...
    if viz:
        ...
        viz_similarities(
            ...
            dict_video_path=dict_video_path,
        )
        # Generate a gif
        if gen_gif:
